...
RESERVE = float(os.environ.get("ERIS_RESERVE", "260"))
...
for f in range(nf):
    el = time.time() - T0
    if (BUDGET - el - RESERVE) < sec_ep * MIN_EPOCH * 1.2:
        break
    ep_f = int(min(MAX_EPOCH, max(MIN_EPOCH, (BUDGET - el - RESERVE) / max(1e-6, sec_ep * (nf - f)))))
    tri = np.concatenate([folds[i] for i in range(NFOLD) if i != f])
    print("extra-seed fold %d epochs %d t=%.0f" % (f, ep_f, time.time() - T0), flush=True)
    models.append(train_fold(tri, ep_f, SEED + 7777 + 100 * f))
    excl.append(f)
...
if K > 0:
    ...
    _need = _per * ((K - 1) * len(train) + K * len(test)) * 1.15
...
try:
    np.savez_compressed(os.path.join(os.path.dirname(os.path.abspath(OUT)) or ".", "oof_probs.npz"),
                        OP=OP[oof], OB=OB[oof], YM=YM[oof], YB=YB[oof])
except Exception:
    pass
