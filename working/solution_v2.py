...
T0 = time.time()
BUDGET = float(os.environ.get("ERIS_BUDGET", "3200"))
...
MIN_EPOCH = int(os.environ.get("ERIS_MIN_EPOCH", "12"))
MAX_EPOCH = int(os.environ.get("ERIS_MAX_EPOCH", "70"))
...
def predict(model, X, rows=None):
    ...
    for s in range(0, n, bs):
        ...
        P[s:s + len(xb)] = (acc_c / 4).cpu().numpy()
        B[s:s + len(xb)] = (acc_b / 4).cpu().numpy()
    return P, B
